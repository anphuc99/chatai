...
export class ChromaClient implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.error({ err: e }, 'Chroma connect failed');
      throw new AppException('CHROMA_UNAVAILABLE', e.message);
    }
  }
  ...
  async addDocuments(docs: MemoryDocument[]): Promise<void> {
    ...
    for (const doc of docs) {
      if (!doc.id || !doc.embedding || doc.embedding.length === 0 || !doc.content || !doc.metadata) {
    ...
    try {
      await this.collection.add({
        ...
        metadatas: docs.map((d) => d.metadata as any), // Cast to any to satisfy chromadb metadata types
      });
    } catch (e: any) {
  ...
  async query(emb: number[], filter: ChromaFilter, k = 5): Promise<MemoryChunk[]> {
    try {
      ...
      return res.ids[0].map((id, i) => ({
        ...
        metadata: res.metadatas && res.metadatas[0] ? (res.metadatas[0][i] as unknown as MemoryMetadata) : ({} as MemoryMetadata),
        distance: res.distances && res.distances[0] && res.distances[0][i] !== null ? (res.distances[0][i] as number) : undefined,
      }));
    } catch (e: any) {
  ...
  async getByIndexRange(
    ...
    try {
      ...
      return res.ids.map((id, i) => ({
        ...
        metadata: res.metadatas ? (res.metadatas[i] as unknown as MemoryMetadata) : ({} as MemoryMetadata),
      }));
    } catch (e: any) {
      throw new AppException('CHROMA_QUERY_FAIL', e.message);
    }
